...
<img src=".\assets\image-20260601204501232.png" alt="image-20260601204501232"  />

- ## langchain-core（核心抽象层 / 基石）
...
1. 在`.env` 文件中添加如下配置
   ...
   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
   ```

   > 注意：功能代码中不需要进行任何额外编码，本地代码运行后，就可以直接在线查看运行记录了

   ![image-20260531075118154](./assets\image-20260531075118154.png)
...
<img src=".\assets\image-20260605230424307.png" alt="image-20260605230424307" style="zoom:80%;" />
...
<img src="./assets\image-20260604190518897.png" alt="image-20260604190518897" style="zoom:50%;" />
...
# 任务：同时触发两种存储路径
user_input = "1. 创建本地文件 local.txt，内容'本地文件内容...'。\n2. 创建文件 /store/memory.txt，内容'重要记忆...'。"
...
# 2. 后端 B: Store 存储 (存重要记忆)
store_backend = StoreBackend(
    namespace=lambda rt: ("deepagents_test",)
)

# 3. 组合后端: 默认走本地文件系统，/store/ 路径走 StoreBackend
...
agent = create_deep_agent(
    ...
    backend=composite_backend,  # 直接传 backend 实例，避免 backend 工厂弃用警告
    tools=[],
    system_prompt="""你是一个智能助手。
    - 普通文件：直接写入文件名（如 `report.txt`），保存到本地工作区。
    - 重要记忆：写入 `/store/` 目录（如 `/store/profile.txt`），保存到store指定的存储方式中。
...
items = store.search(("deepagents_test",))
for item in items:
    # print(item)
...
# 1. 创建一个fileSystemBackend来设置skill所在的文件夹
...
result = main_agent.invoke(
    {
        "messages": [
            # {"role": "user", "content": "帮我罗列一下，我有哪些技能"}
            {"role": "user", "content": "将‘我今天要坐飞机去北京潇洒一下’翻译为表情符"},
            # {"role": "assistant", "content": "审查一下我当前项目下08_skill.py的代码"}
        ]
    },
)
